...
| 10  | **Results view** — chain result cards with resource bar, implicit outputs, deficit overflow indicator; tap-to-expand detail table; sort selector; "Load more" pagination (appends next page); `cap_reached` notice; Phase 1 and Phase 2 failure states                                                            | 7, 9    | Vitest: cards render with correct bar scaling; deficit card shows overflow + warning symbol; detail table renders correct rows; "Load more" appends without replacing; sort selector triggers correct API call; failure states render correct messages | done (`0a41498`)   |
